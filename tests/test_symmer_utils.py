...
H2_sto3g = {'qubit_encoding': 'jordan_wigner',
 ...
 'calculated_properties': {'HF': {'energy': -1.1167593073964255,
   'converged': True},
 ...
 'auxiliary_operators': {'number_operator': {'IIII': (2.0, 0.0),
   'IIIZ': (-0.5, 0.0),
   'IIZI': (-0.5, 0.0),
   'IZII': (-0.5, 0.0),
   'ZIII': (-0.5, 0.0)},
  'S^2_operator': {'IIII': (0.75, 0.0),
   'IIIZ': (0.5, 0.0),
   'IIZI': (-0.5, 0.0),
   'IIZZ': (-0.375, 0.0),
   'IZII': (0.5, 0.0),
   ...
   'YYXX': (0.125, 0.0),
   'YYYY': (0.125, 0.0)},
  'Sz_operator': {'IIIZ': (0.25, 0.0),
   'IIZI': (-0.25, 0.0),
   'IZII': (0.25, 0.0),
   'ZIII': (-0.25, 0.0)},
  'alpha_parity_operator': {'ZIZI': (1.0, 0.0)},
  'beta_parity_operator': {'IZIZ': (1.0, 0.0)},
  'MP2_operator': {'XXXX': (-0.004531358676614097, 0.0),
  ...
  'CCSD_operator': {'XXXX': (-0.007079023951543804, 0.0),
 ...
 'H_dict': {'IIII': (-0.09706626816763123+0j),
...
He3_plus = {'qubit_encoding': 'jordan_wigner',
 'unit': 'angstrom',
 'geometry': '3\n \nH\t0\t0\t0\nH\t0\t0\t0.74\nH\t0\t0\t1.48',
 'basis': 'STO-3G',
 'charge': 1,
 ...
 'convergence_threshold': 1e-06,
 'point_group': {'groupname': 'Dooh', 'topgroup': 'Dooh'},
 'calculated_properties': {'HF': {'energy': -1.189999028637302,
   'converged': True},
 ...
 'auxiliary_operators': {'number_operator': {'IIIIII': (3.0, 0.0),
   'IIIIIZ': (-0.5, 0.0),
   'IIIIZI': (-0.5, 0.0),
   'IIIZII': (-0.5, 0.0),
   'IIZIII': (-0.5, 0.0),
  ...
  'MP2_operator': {'XXIIXX': (-0.0022135760877754116, 0.0),
  ...
  'CCSD_operator': {'IXZZZX': (-0.0031353836158880214, 0.0),
 ...
 'H_dict': {'IIIIII': (0.24747487013571695+0j),
...
def test_exact_gs_energy_H3_plus():
    ...
    num_operator = PauliwordOp.from_dictionary(He3_plus['auxiliary_operators']['number_operator'])
    n_part = He3_plus['n_particles']['total']

    gs_energy, gs_state = exact_gs_energy(H.to_sparse_matrix,
                                          number_operator=num_operator,
                                          n_particles=n_part)
...
def test_random_anitcomm_2n_1_PauliwordOp_method():
    ...
    AC = random_anitcomm_2n_1_PauliwordOp(n_qubits,
...
def test_random_anitcomm_2n_1_PauliwordOp_real_with_clifford():
    ...
    anti_comm_check = AC.adjacency_matrix.astype(int) - np.eye(AC.adjacency_matrix.shape[0])
    assert(np.sum(anti_comm_check) == 0), 'operator not made up of pairwisie anticommuting Pauli operators'
...
def test_gram_schmidt_from_quantum_state_QuantumState():
    ...
    assert np.allclose(U_gram[:, 0], psi.to_sparse_matrix.toarray().reshape([-1])), 'first column of U_gram not correct'
