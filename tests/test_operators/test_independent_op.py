...
def test_commuting_overide_symmetry_generators():
    op = PauliwordOp.from_list(["IZZ", "ZZI", "IXX", "XXI", "IYY", "YYI"])
    gen_without_override = IndependentOp.symmetry_generators(
        op, commuting_override=False
    )
    ...
    assert gen_without_override in [
        IndependentOp.from_list(["XXX"]),
        IndependentOp.from_list(["ZZZ"]),
    ]
...
def test_rotations_onto_sqp_Z():
    ...
    assert np.all(np.sum(rotated.Z_block, axis=1) <= 1) and np.all(~rotated.X_block)
...
def test_symmetry_generators_H2():
    ...
    assert np.all(G1.generator_reconstruction(G2)[1]) and np.all(
        G2.generator_reconstruction(G1)[1]
    )
...
def test_value_assignment():
    ...
    assert np.all(
        G.coeff_vec == (-1) ** np.sum(np.bitwise_and(G.Z_block, ref_state), axis=1)
    )
