...
def test_init_not_anticommuting():
    ...
    with pytest.raises(AssertionError):
        AntiCommutingOp.from_dictionary({"ZZZ": 1, "ZIZ": 1, "ZZI": -1, "III": 1})
...
def test_init_commuting():
    ...
    check assert error thrown if input is not anticommuting
    """
    AcOp = AntiCommutingOp.from_dictionary({"ZZZ": 1, "XXX": 1, "YYY": -1})
    P = PauliwordOp.from_list(["ZZZ", "XXX", "YYY"], [1, 1, -1])
    assert np.allclose(AcOp.to_sparse_matrix.toarray(), P.to_sparse_matrix.toarray())
...
def test_unitary_partitioning_no_s_index_seq_rot():
    ...
    Ps, rotations, gamma_l, AC_op = AcOp_real.unitary_partitioning(
        s_index=None, up_method="seq_rot"
    )

    assert Ps.n_terms == 1, "can only rotate onto single Pauli operator"
    assert gamma_l == np.linalg.norm(
        list(anti_commuting_real.values())
    ), "normalization wrong"
    ...
    for X_sk, theta_sk in rotations[::-1]:
        ...
        assert (
            X_sk.n_terms == 1
        ), f"rotation generated  by single pauli operator only! Not {X_sk.n_terms}"

        # Let R(t) = e^{i t/2 Q} = cos(t/2)*I + i*sin(t/2)*Q
        R = PauliwordOp.from_dictionary(
            {"I" * AcOp_real.n_qubits: np.cos(theta_sk / 2)}
    ...
    assert R_seq_rot_Op * R_seq_rot_Op.dagger == PauliwordOp.from_dictionary(
        {"I" * AcOp_real.n_qubits: 1}
    ), "R not unitary"
...
def test_unitary_partitioning_no_s_index_LCU():
    ...
    assert (
        rotations.n_terms == AcOp_real.n_terms
    ), "LCU op contains too many pauli operators"

    R_AC_op_Rdag = rotations * AcOp_real * rotations.dagger
    assert R_AC_op_Rdag == Ps.multiply_by_constant(gamma_l)
    assert rotations * rotations.dagger == PauliwordOp.from_dictionary(
        {"I" * AcOp_real.n_qubits: 1}
    ), "R not unitary"
...
def test_unitary_partitioning_s_index_seq_rot():
    ...
    for s_ind in range(AcOp_real.n_terms):
        Ps, rotations, gamma_l, AC_op = AcOp_real.unitary_partitioning(
            s_index=s_ind, up_method="seq_rot"
        )
        ...
        assert (
            len(rotations) == AcOp_real.n_terms - 1
        ), "seq of rotation number incorrect"
...
def test_unitary_partitioning_seq_rot_complex():
    ...
    with pytest.raises(AssertionError):
        AcOp_comp.unitary_partitioning(s_index=None, up_method="seq_rot")


def test_unitary_partitioning_LCU_complex():
    AcOp_comp = AntiCommutingOp.from_dictionary(anti_commuting_complex)
    with pytest.raises(AssertionError):
        AcOp_comp.unitary_partitioning(s_index=None, up_method="LCU")
...
def test_ac_set_with_zero_ceoffs():
    ...
    Ps_seq_rot, rotations_seq_rot, gamma_l, AC_normed = AcOp_real.unitary_partitioning(
        s_index=1, up_method="seq_rot"
    )
    ...
    Ps_LCU, rotations_LCU, gamma_l, AC_normed = AcOp_real.unitary_partitioning(
        s_index=1, up_method="LCU"
    )
