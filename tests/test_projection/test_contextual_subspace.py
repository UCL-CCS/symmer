...
from symmer import ContextualSubspace, QuantumState, QubitTapering
from symmer.evolution import trotter
from symmer.operators import IndependentOp, NoncontextualOp, PauliwordOp
...
def test_update_stabilizers_HOMO_LUMO_biasing():
    ...
    CS.update_stabilizers(
        3,
        aux_operator=CC_taper,
        strategy="HOMO_LUMO_biasing",
    ...
    for _ in range(10):
        ...
        samples.append(
            abs(
                exact_gs_energy(CS.project_onto_subspace().to_sparse_matrix)[0]
                - fci_energy
            )
        )
...
@pytest.mark.parametrize(
    "ref_state", [QT.tapered_ref_state, QT.tapered_ref_state.state_matrix[0]]
)
def test_reference_state(ref_state):
    CS = ContextualSubspace(
        H_taper, noncontextual_strategy="StabilizeFirst", reference_state=ref_state
    )
    CS.update_stabilizers(3, aux_operator=CC_taper, strategy="aux_preserving")
    H_cs = CS.project_onto_subspace()
    assert H_cs.n_qubits == 3
...
def test_project_auxiliary_operator():
    ...
    assert (
        abs(
            H_cs.expval(trotter(CC_cs * 1j, trotnum=10) * QuantumState([0, 0, 0]))
            - fci_energy
        )
        < 0.0004
    )
