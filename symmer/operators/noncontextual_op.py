...
class NoncontextualOp(PauliwordOp):
    ...
    def __init__(self, symp_matrix, coeff_vec):
        """
        Args:
    ...
    def from_PauliwordOp(cls, H) -> "NoncontextualOp":
        ...
        """
        noncontextual_operator = cls(H.symp_matrix, H.coeff_vec)
    ...
    def from_hamiltonian(
        ...
        if strategy == "diag":
            ...
            return cls._from_generators_noncontextual_op(
                H, generators, use_jordan_product=use_jordan_product
            )
        elif strategy == "stabilizers":
            return cls._from_stabilizers_noncontextual_op(
                H, stabilizers, use_jordan_product=use_jordan_product
            )
    ...
    def _diag_noncontextual_op(cls, H: PauliwordOp) -> "NoncontextualOp":
        ...
        noncontextual_operator = cls(H.symp_matrix[mask_diag], H.coeff_vec[mask_diag])
        return noncontextual_operator

    @classmethod
    def _dfs_noncontextual_op(
        cls, H: PauliwordOp, runtime=10, strategy="magnitude"
    ) -> "NoncontextualOp":
        """
        ...
        if strategy == "magnitude":
            noncontextual_operator = sorted(
                noncontextual_ops, key=lambda x: -np.sum(abs(x.coeff_vec))
            )[0]
        elif strategy == "largest":
            noncontextual_operator = sorted(
                noncontextual_ops, key=lambda x: -x.n_terms
            )[0]
        else:
    ...
    @classmethod
    def _single_sweep_noncontextual_operator(
        cls, H, strategy="magnitude"
    ) -> "NoncontextualOp":
        """
        ...
        if strategy == "magnitude":
            ...
            operator = PauliwordOp(H.symp_matrix[order], H.coeff_vec[order])
        elif strategy == "CurrentOrder":
            operator = H
        else:
            raise ValueError(
                "Unrecognised strategy, must be one of magnitude, random or CurrentOrder"
            )
    ...
    def _from_generators_noncontextual_op(
        ...
        if use_jordan_product:
            ...
            _, noncontextual_terms_mask = H.generator_reconstruction(
                generators, override_independence_check=True
            )
    ...
    def random(
        ...
        assert (
            n_cliques > 1
        ), "number of cliques must be set to 2 or more (cannot have one anticommuting term)"
        n_clique_qubits = int(np.ceil((n_cliques - 1) / 2))
        assert (
            n_clique_qubits <= n_qubits
        ), "cannot have {n_cliques} anticommuting cliques on {n_qubits} qubits"
        ...
        if n_commuting_terms:
            ...
            ), f"cannot have {n_commuting_terms} commuting operators on {remaining_qubits} qubits"
        ...
        if remaining_qubits >= 1:
            if n_commuting_terms == None:
                ...
                XZ_block = (
                    (
                        (
                            np.arange(n_commuting_terms)[:, None]
                            & (1 << np.arange(2 * remaining_qubits))[::-1]
                        )
                    )
                    > 0
                ).astype(bool)
                ...
                indices = np.unique(
                    np.random.random_integers(
                        0, high=2**remaining_qubits - 1, size=10 * n_commuting_terms
                    )
                )
                ...
                XZ_block = (
                    ((indices[:, None] & (1 << np.arange(2 * remaining_qubits))[::-1]))
                    > 0
                ).astype(bool)
        ...
        if n_cliques == 0:
            ...
            AC = random_anitcomm_2n_1_PauliwordOp(n_clique_qubits, apply_clifford=True)[
                :n_cliques
            ]
            ...
            H_nc = AC_full * H_sym
            assert (
                AC.n_terms * n_commuting_terms == H_nc.n_terms
            ), "operator not largest it can be"
    ...
    @classmethod
    def _from_stabilizers_noncontextual_op(
        cls, H: PauliwordOp, stabilizers: IndependentOp, use_jordan_product=False
    ) -> "NoncontextualOp":
        """
        Args:
        ...
        """
        symmetries = IndependentOp.symmetry_generators(
            stabilizers, commuting_override=True
        )
        ...
        return cls._from_generators_noncontextual_op(
            H=H, generators=generators, use_jordan_product=use_jordan_product
        )
    ...
    def noncontextual_generators(self) -> None:
        ...
        if not np.all(Z2_symmerties.commutes_termwise(Z2_symmerties)):
            ...
            z2_mask = (
                np.sum(sym_gens.commutes_termwise(sym_gens), axis=1) == sym_gens.n_terms
            )
            ...
            sym_from_cliques = sum(
                (cover[n] - C_rep) * C_rep
                for n, C_rep in enumerate(clique_rep_list)
                if cover[n].n_terms > 1
            )
        ...
        if remaining.n_terms > 0:
            ...
            self.decomposed = {
                ind: remaining[c_mask] for ind, c_mask in enumerate(clique_mask)
            }
            ...
            if self.n_cliques > 0:
                ...
                self.clique_operator.coeff_vec = np.ones_like(
                    self.clique_operator.coeff_vec
                )

                ## cliques can form new Z2 syms
                sym_from_cliques = sum(
                    (self.decomposed[n] - C_rep) * C_rep
                    for n, C_rep in enumerate(clique_rep_list)
                    if self.decomposed[n].n_terms > 1
                )
                if sym_from_cliques:
    ...
    def noncontextual_reconstruction(self) -> None:
        ...
        noncon_generators = PauliwordOp(
            np.vstack(
                [self.symmetry_generators.symp_matrix, self.clique_operator.symp_matrix]
            ),
        ...
        jordan_recon_matrix, successful = self.jordan_generator_reconstruction(
            noncon_generators
        )  # , override_independence_check=True)
        assert np.all(
            successful
        ), "The generating set is not sufficient to reconstruct the noncontextual Hamiltonian"
    ...
    def symmetrized_operator(self, expansion_order=1):
        ...
        terms = [
            (I - S) ** n * (-1) ** n * binomial_coefficient(0.5, n)
    ...
    def get_symmetry_contributions(self, nu: np.array) -> float:
        ...
        coeff_mod = (
            ...
            *
            ...
            (-1)
            ** np.count_nonzero(np.logical_and(self.G_indices == 1, nu == -1), axis=1)
        )
    ...
    def update_clique_representative_operator(
        self, clique_index: int = None
    ) -> List[Tuple[PauliwordOp, float]]:
        ...
        ) = self.clique_operator.unitary_partitioning(
            up_method=self.up_method, s_index=clique_index
        )
    ...
    def noncon_state(
        self, UP_method: Optional[str] = "LCU"
    ) -> Tuple[QuantumState, np.array]:
        """
        ...
        if UP_method == "LCU":
            (
                ...
                gamma_l,
                AC_normed,
            ) = self.clique_operator.unitary_partitioning(s_index=0, up_method="LCU")
        else:
            (
                Ps,
                rotations_SEQ,
        ...
        Z_vals = np.sum(
            rotated_stabs.Z_block[:, Z_indices.astype(bool)] * rotated_stabs.coeff_vec,
            axis=1,
        )
        ...
        if UP_method == "LCU":
            ...
            for op, angle in rotations_SEQ[::-1]:
                state = (
                    exponentiate_single_Pop(
                        op.multiply_by_constant(1j * angle / 2)
                    ).dagger
                    * state
                )
...
class NoncontextualSolver:
    ...
    def __init__(
        ...
        if fixed_ev_mask is not None:
            ...
            assert np.sum(fixed_ev_mask) == len(
                fixed_eigvals
            ), "Number of non-zero elements in mask does not match the number of fixed eigenvalues"
    ...
    def energy_via_brute_force(self) -> Tuple[float, np.array, np.array]:
        ...
        if np.all(self.fixed_ev_mask):
            ...
            nu_list = np.ones(
                [search_size, self.NC_op.symmetry_generators.n_terms], dtype=int
            )
            nu_list[:, self.fixed_ev_mask] = np.tile(
                self.fixed_eigvals, [search_size, 1]
            )
            nu_list[:, ~self.fixed_ev_mask] = np.array(
                list(itertools.product([-1, 1], repeat=np.sum(~self.fixed_ev_mask)))
            )

        # # optimize over all discrete value assignments of nu in parallel
        noncon_H = ray.put(self.NC_op)
        tracker = np.array(
            ray.get([get_noncon_energy.remote(noncon_H, nu_vec) for nu_vec in nu_list])
        )
    ...
    def energy_via_relaxation(self) -> Tuple[float, np.array, np.array]:
        ...
        nu_bounds = [(0, np.pi)] * (
            self.NC_op.symmetry_generators.n_terms - np.sum(self.fixed_ev_mask)
        )
        ...
        optimizer_output = shgo(
            func=lambda angles: self.NC_op.get_energy(get_nu(angles)), bounds=nu_bounds
        )
    ...
    def get_cost_func(self):
        ...
        """
        symmetrized_operator = self.NC_op.symmetrized_operator(
            expansion_order=self.expansion_order
        )
        G_indices, _ = symmetrized_operator.generator_reconstruction(
            self.NC_op.symmetry_generators
        )
    ...
    def energy_xUSO(self) -> Tuple[float, np.array, np.array]:
        ...
        if np.all(self.fixed_ev_mask):
            ...
            if self.method == "brute_force":
                ...
                if self.x == "P":
                    puso_res = qv.sim.anneal_puso(
                        spin_problem, num_anneals=self.num_anneals
                    )
                elif self.x == "Q":
                    puso_res = qv.sim.anneal_quso(
                        spin_problem, num_anneals=self.num_anneals
                    )
            ...
            nu_vec[~self.fixed_ev_mask] = np.array(
                [solution[x_i] for x_i in sorted(COST.variables)]
            )
