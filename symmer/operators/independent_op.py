...
from symmer.operators.utils import _cref_binary, _rref_binary, check_independent
...
class IndependentOp(PauliwordOp):
    ...
    def __init__(
        ...
        if target_sqp in ["X", "Z", "Y"]:
            ...
            raise ValueError(
                "Target single-qubit Pauli not recognised - must be X or Z"
            )
    ...
    @classmethod
    def from_PauliwordOp(cls, PwordOp: PauliwordOp) -> "IndependentOp":
        """
        Args:
    ...
    def symmetry_generators(
        ...
        to_reduce = np.vstack(
            [
                np.hstack([PwordOp.Z_block, PwordOp.X_block]),
                np.eye(2 * PwordOp.n_qubits, dtype=bool),
            ]
        )
        cref_matrix = _cref_binary(to_reduce)
        S_symp = cref_matrix[
            PwordOp.n_terms :, np.all(~cref_matrix[: PwordOp.n_terms], axis=0)
        ].T
        ...
        if np.all(S.adjacency_matrix) or commuting_override:
            ...
            if S.n_terms < 10 or largest_clique:
                ...
                warnings.warn(
                    "Greedy method may identify non-optimal commuting symmetry terms; might be able to taper again."
                )

            return cls(
                S_commuting.symp_matrix, np.ones(S_commuting.n_terms, dtype=complex)
            )
    ...
    def _recursive_rotations(self, basis: "IndependentOp") -> None:
        ...
        basis_non_sqp = IndependentOp(
            basis.symp_matrix[non_sqp], basis.coeff_vec[non_sqp]
        )
        sqp_indices = np.where((basis - basis_non_sqp).symp_matrix)[1] % self.n_qubits
        self.used_indices += np.append(
            sqp_indices, sqp_indices + self.n_qubits
        ).tolist()
        ...
        if basis_non_sqp.n_terms == 0:
            ...
            target[
                pivot_point + self.n_qubits * (-1) ** (pivot_point // self.n_qubits)
            ] = 1
    ...
    def generate_stabilizer_rotations(self) -> None:
        ...
        """
        assert (
            self.n_terms <= self.n_qubits
        ), "Too many terms in basis to reduce to single-qubit Paulis"
        assert np.all(
            self.adjacency_matrix
        ), "The basis is not commuting, hence the rotation is not possible"
    ...
    def update_sector(
        ...
        if np.any(self.coeff_vec == 0):
            S_zero = self[self.coeff_vec == 0]
            S_zero.coeff_vec[:] = 1
            S_zero = list(S_zero.to_dictionary.keys())
            warnings.warn(
                f"The stabilizers {S_zero} were assigned zero values - bad reference state."
            )
    ...
    def __getitem__(self, key: Union[slice, int]) -> "IndependentOp":
        """
