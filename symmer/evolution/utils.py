...
def get_CNOT_connectivity_graph(
    evolution_obj: Union[PauliwordOp, QuantumCircuit], print_graph=False
):
    """
    ...
    if isinstance(evolution_obj, QuantumCircuit):
        edges = [
            [q.index for q in step[1]]
            for step in evolution_obj.data
            if step[0].name != "barrier" and len(step[1]) > 1
        ]
        weighted_edges = [(u, v, w) for (u, v), w in Counter(edges).items()]
    else:
        rows, cols = np.where(evolution_obj.X_block | evolution_obj.Z_block)
        support_indices = [
            evolution_obj.n_qubits - 1 - cols[rows == i] for i in np.unique(rows)
        ]
    ...
    if depth == 0:
        ...
        for nodes in ordered_nodes:
            ...
            if GraphMatcher(target, G_temp).subgraph_is_isomorphic():
                dropped_edge_weights = [
                    G.edges[e]["weight"] for n in nodes for e in G.edges(n)
                ]
...
def topology_match_score(ansatz_operator, topology, max_depth=3):
    ...
    subgraph_cost = subgraph_isomorphism_distance(
        entangling_graph, topology, max_depth=max_depth
    )
    n_entangling_gates = np.count_nonzero(
        ansatz_operator.X_block | ansatz_operator.Z_block
    )
