...
class VQE_Driver:
    ...
    def prepare_for_evolution(self, excitation_ops: PauliwordOp) -> None:
        ...
        self.circuit = PauliwordOp_to_QuantumCircuit(
            PwordOp=self.excitation_generators,
            ref_state=self.ref_state,
            bind_params=False,
        )

    def get_state(
        self, evolution_obj: Union[QuantumCircuit, PauliwordOp], x: np.array
    ...
    def _f(
        ...
        if self.expectation_eval == "dense_array":
            return (
                state.conjugate().T @ observable.to_sparse_matrix.toarray() @ state
            )[0, 0].real
        elif self.expectation_eval == "sparse_array":
            return (state.conjugate().T @ observable.to_sparse_matrix @ state)[
                0, 0
            ].real
            ...
            return (
                self.ref_state.dagger
                * observable.perform_rotations(state)
                * self.ref_state
            ).real
    ...
    def partial_derivative(self, x: np.array, param_index: int) -> float:
        ...
        """
        x_upper = x.copy()
        x_upper[param_index] += np.pi / 4
        x_lower = x.copy()
        x_lower[param_index] -= np.pi / 4
    ...
    def run(self, x0: np.array = None, **kwargs):
        ...
        opt_out = minimize(fun=fun, jac=jac, x0=x0, **kwargs)
...
class ADAPT_VQE(VQE_Driver):
    ...
    def _derivative_from_param_shift(self, index):
        ...
        upper_state = self.get_state(
            circuit_temp, np.append(self.opt_parameters, +np.pi / 4)
        )
        lower_state = self.get_state(
            circuit_temp, np.append(self.opt_parameters, -np.pi / 4)
        )
        return self._f(self.observable, upper_state) - self._f(
            self.observable, lower_state
        )
        ...
        if self.derivative_eval == "commutators":
            ...
            if self.expectation_eval == "observable_rotation":
                self.current_state = self.get_state(
                    self.adapt_operator, self.opt_parameters
                )
            else:
                circuit_temp = PauliwordOp_to_QuantumCircuit(
                    PwordOp=self.adapt_operator,
                    ref_state=self.ref_state,
                    bind_params=False,
                )
                self.current_state = self.get_state(circuit_temp, self.opt_parameters)
            if self.expectation_eval in [
                "sparse_array",
                "symbolic_direct",
                "observable_rotation",
            ]:
                ...
                with mp.Pool(mp.cpu_count()) as pool:
                    gradient = pool.map(
                        self._derivative_from_commutators,
                        range(self.excitation_pool.n_terms),
                    )
            else:
            ...
            gradient = list(
                map(
                    self._derivative_from_param_shift,
                    range(self.excitation_pool.n_terms),
                )
            )
            ...
            for index in range(self.excitation_pool.n_terms):
                ...
                linearity_scores.append(
                    (
                        ...
                        - len([a for b in cycle_basis(connectivity) for a in b])
                        * self.bias
                    )
                    / circuit_temp.num_qubits
                )
    ...
    def optimize(
        ...
        while (
            gmax > gtol
            and adapt_cycle <= max_cycles
            and abs(anew - aold) > atol
            and abs(anew - target) > target_error
        ):
            ...
            if self.TETRIS:
                ...
                for i in grad_rank:
                    ...
                    support_exists = (
                        new_excitation.X_block | new_excitation.Z_block
                    ) & support_mask
            ...
            interim_data[adapt_cycle] = {
                ...
                "gmax": gmax,
                "excitation": [
                    symplectic_to_string(t.symp_matrix[0]) for t in new_excitation_list
                ],
            }
            anew = opt_out["fun"]
        ...
        return {
            ...
            "adapt_operator": [
                symplectic_to_string(t) for t in self.adapt_operator.symp_matrix
            ],
        }
...
def serialize_opt_data(opt_data):
    return {
        "message": opt_data.message,
        "success": opt_data.success,
        "status": opt_data.status,
        "fun": opt_data.fun,
        "x": tuple(opt_data.x),
        "jac": tuple(opt_data.jac),
        "nit": opt_data.nit,
        "nfev": opt_data.nfev,
        "njev": opt_data.njev,
    }
