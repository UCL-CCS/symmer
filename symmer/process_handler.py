...
class ProcessHandler:
    ...
    def _process_ray(self, func, iter, shared):
        ...
        results = get(
            [_func.remote(chunk, shared_obj) for chunk in self.prepare_chunks(iter)]
        )
    ...
    def parallelize(self, func):
        def wrapper(iter, shared):
            ...
            if self.method == "mp":
                ...
                raise ValueError(
                    f"Invalid processing method {self.method}, must be ray, mp or single_thread."
                )
