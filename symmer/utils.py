...
def exact_gs_energy(
    ...
    if n_particles is None:
        ...
        for evl, evc in zip(eigvals, eigvecs.T):
            psi = QuantumState.from_array(evc.reshape([-1, 1])).cleanup(
                zero_threshold=1e-5
            )
            assert ~np.any(number_operator.X_block), "Number operator not diagonal"
            expval_n_particle = 0
            for Z_symp, Z_coeff in zip(
                number_operator.Z_block, number_operator.coeff_vec
            ):
                sign = (-1) ** np.einsum(
                    "ij->i", np.bitwise_and(Z_symp, psi.state_matrix)
                )
                expval_n_particle += Z_coeff * np.sum(
                    sign * np.square(abs(psi.state_op.coeff_vec))
                )
        ...
        raise RuntimeError(
            "No eigenvector of the correct particle number was identified - try increasing n_eigs."
        )


def random_anitcomm_2n_1_PauliwordOp(
    n_qubits, complex_coeff=False, apply_clifford=True
):
    """
...
def get_sparse_matrix_large_pauliwordop(P_op: PauliwordOp) -> csr_matrix:
    ...
    if nq < 16:
        ...
        if (n_chunks <= 1) or (P_op.n_terms <= 1):
            ...
            n_chunks += 1
            P_op_chunks_inds = (
                np.rint(np.linspace(0, P_op.n_terms, min(n_chunks, P_op.n_terms + 1)))
                .astype(set)
                .astype(int)
            )
            P_op_chunks = [
                P_op[P_op_chunks_inds[ind_i] : P_op_chunks_inds[ind_i + 1]]
                for ind_i, _ in enumerate(P_op_chunks_inds[1:])
            ]
            tracker = np.array(
                ray.get(
                    [
                        _get_sparse_matrix_large_pauliwordop.remote(op)
                        for op in P_op_chunks
                    ]
                )
            )
...
def _get_sparse_matrix_large_pauliwordop(P_op: PauliwordOp) -> csr_matrix:
    ...
    for op in P_op:
        left_tensor = np.hstack((op.X_block[:, : nq // 2], op.Z_block[:, : nq // 2]))
        left_coeff = op.coeff_vec

        right_tensor = np.hstack((op.X_block[:, nq // 2 :], op.Z_block[:, nq // 2 :]))
        right_coeff = np.array([1])

        mat += sparse_kron(
            PauliwordOp(left_tensor, left_coeff).to_sparse_matrix,
            PauliwordOp(right_tensor, right_coeff).to_sparse_matrix,
            format="csr",
        )  # setting format makes this faster!

    return mat


def matrix_allclose(
    A: Union[csr_matrix, np.array], B: Union[csr_matrix, np.array], tol: int = 1e-15
) -> bool:
    """
...
def get_PauliwordOp_root(power: int, pauli: PauliwordOp) -> PauliwordOp:
    ...
    Pk = I_term.multiply_by_constant(
        cos_term**2 + 1j * cos_term * sin_term
...
def get_generators_including_xz_products(operator: PauliwordOp) -> PauliwordOp:
    ...
    sym_final = np.hstack((np.logical_or(X_new, Y_new), np.logical_or(Z_new, Y_new)))
