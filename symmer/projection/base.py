...
from symmer.evolution import Had, trotter
...
class S3Projection:
    ...
    def __init__(self, stabilizers: IndependentOp) -> None:
        """
    ...
    def _perform_projection(
        ...
        """
        assert (
            operator.n_qubits == self.stabilizers.n_qubits
        ), "The input operator does not have the same number of qubits as the stabilizers"
        assert (
            self.rotated_flag
        ), "The operator has not been rotated - intended for use with perform_projection method"
        self.rotated_flag = False

        # remove terms that do not commute with the rotated stabilizers
        commutes_with_all_stabilizers = np.all(
            operator.commutes_termwise(self.rotated_stabilizers), axis=1
        )
        ...
        eigval_assignment = (
            op_anticommuting_removed[:, stab_symp_indices]
            * self.rotated_stabilizers.coeff_vec
        )
        eigval_assignment[
            eigval_assignment == 0
        ] = 1  # 0 entries are identity, so fix as 1 in product
        coeff_sign_flip = (
            cf_anticommuting_removed * (np.prod(eigval_assignment, axis=1)).T
        )

        # the projected Pauli terms:
        unfixed_XZ_indices = np.hstack(
            [self.free_qubit_indices, self.free_qubit_indices + operator.n_qubits]
        )
    ...
    def perform_projection(
        ...
        self.stab_qubit_indices = (
            np.where(self.rotated_stabilizers.symp_matrix)[1] % operator.n_qubits
        )
        self.free_qubit_indices = np.setdiff1d(
            np.arange(operator.n_qubits), self.stab_qubit_indices
        )

        # perform the full list of rotations on the input operator...
        if len(self.stabilizers.stabilizer_rotations) > 0:
            op_rotated = operator.perform_rotations(
                self.stabilizers.stabilizer_rotations
            )
        else:
            op_rotated = operator
    ...
    def project_state(self, state: QuantumState) -> QuantumState:
        ...
        transformation_list += [
            Had(self.stabilizers.n_qubits, i)
            for i in np.where(
                np.sum(
        ...
        transformation_list += list(
            map(
                lambda s: trotter(s[0] * (np.pi / 4 * 1j)),
                self.stabilizers.stabilizer_rotations,
            )
        )
