...
from symmer.operators import IndependentOp, NoncontextualOp, PauliwordOp, QuantumState
from symmer.projection import S3Projection
from symmer.projection.utils import (  # get_noncon_generators_from_commuting_stabilizers
    ObservableBiasing,
    StabilizerIdentification,
    stabilizer_walk,
    update_eigenvalues,
)
...
class ContextualSubspace(S3Projection):
    ...
    def update_stabilizers(
        ...
        """
        assert (
            n_qubits <= self.operator.n_qubits
        ...
        if n_qubits == self.operator.n_qubits:
            ...
            if strategy == "aux_preserving":
                ...
                S = self._random_stabilizers(n_qubits=n_qubits)
            elif strategy == "HOMO_LUMO_biasing":
                S = self._HOMO_LUMO_biasing(
                    n_qubits=n_qubits,
                    HF_array=HF_array,
                    weighting_operator=aux_operator,
    ...
    def _noncontextual_update(self):
        ...
        if self.noncontextual_operator is not None:
            ...
            if self.contextual_operator.n_terms == 0:
                raise ValueError(
                    "The Hamiltonian is noncontextual, the contextual subspace is empty."
                )
    ...
    def _aux_operator_preserving_stabilizer_search(
        self, n_qubits: int, aux_operator: PauliwordOp, use_X_only: bool = True
    ) -> IndependentOp:
        """
    ...
    def _HOMO_LUMO_biasing(
        ...
        """
        assert (
            HF_array is not None
        ), "Must supply the Hartree-Fock state for this strategy"

        OB = ObservableBiasing(
            base_operator=self.operator,
            HOMO_LUMO_gap=np.where(np.asarray(HF_array == 0).reshape(-1))[0][0]
            - 0.5,  # currently assumes JW mapping!
        )
        S = stabilizer_walk(
    ...
    def _random_stabilizers(self, n_qubits: int) -> IndependentOp:
        """
    ...
    def _prepare_stabilizers(self) -> None:
        ...
        if self.nc_strategy == "StabilizeFirst":
            self.noncontextual_operator = (
                NoncontextualOp._from_stabilizers_noncontextual_op(
                    H=self.operator,
        ...
        if self.noncontextual_operator.n_cliques > 0:
            ...
            clique_commutation = self.stabilizers.commutes_termwise(
                self.noncontextual_operator.clique_operator
            )
        ...
        if ~np.all(mask_which_clique):
            ...
            augmented_generators = (
                IndependentOp(
                    self.noncontextual_operator.mapped_clique_rep.symp_matrix, [-1]
                )
            ...
            update_eigenvalues(
                generators=augmented_generators, stabilizers=self.stabilizers
            )
    ...
    def project_onto_subspace(
        self, operator_to_project: PauliwordOp = None
    ) -> PauliwordOp:
        """
        ...
        if self.perform_unitary_partitioning:
            ...
            if self.noncontextual_operator.up_method == "LCU":
                ...
                rotated_op = (
                    self.noncontextual_operator.unitary_partitioning_rotations
                    * operator_to_project
                    * self.noncontextual_operator.unitary_partitioning_rotations.dagger
                ).cleanup()
            elif self.noncontextual_operator.up_method == "seq_rot":
                # sequence-of-rotations approach
                rotated_op = operator_to_project.perform_rotations(
                    self.noncontextual_operator.unitary_partitioning_rotations
                )
            else:
                raise ValueError(
                    "Unrecognised unitary partitioning rotation method, must be one of LCU or seq_rot."
                )
        else:
        ...
        if self.return_NC:
            assert (
                cs_operator.n_qubits == 1
            ), "Projected operator consists of more than one qubit."
    ...
    def project_state_onto_subspace(
        self, state_to_project: QuantumState = None
    ) -> QuantumState:
        """
        ...
        assert (
            self.S3_initialized
        ), "Must first project an operator into the contextual subspace via the project_onto_subspace method"
        # can provide an auxiliary state to project, although not in general scalable
        if state_to_project is None:
            assert (
                self.ref_state is not None
            ), "Must provide a state to project into the contextual subspace"
        ...
        if self.perform_unitary_partitioning:
            ...
            if self.noncontextual_operator.up_method == "LCU":
                ...
                rotation_generator = sum(
                    [
                        R * angle * 0.5 * 1j
                        for R, angle in self.noncontextual_operator.unitary_partitioning_rotations
                    ]
                )
